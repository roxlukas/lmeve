...
?>	    <div class="tytul">
	...
	<?php
		...
		$nr=addslashes($nr);
		if (db_count("SELECT `errorID` from `esistatus` WHERE `errorID`=$nr")==0) {
			echo("Such record does not exist.");
			return;
		}
		$do=$_GET['do'];
		
		$status=db_asocquery("SELECT * FROM `esistatus` WHERE `errorID`=$nr");
		$status=$status[0];
			
	if ($do==1) {
			
			db_uquery("UPDATE `esistatus` SET `errorCode`='0', `errorCount`='0' WHERE `errorID`=$nr;");
		...
		<script type="text/javascript">location.href="index.php?id=8&id2=7";</script>
		<?php
	} else {
		?>
